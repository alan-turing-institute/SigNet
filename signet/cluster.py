...
class Cluster:
    ...
    def spectral_cluster_adjacency_reg(self, k=2, normalisation='sym_sep', tau_p=None, tau_n=None, eigens=None, mi=None):
    ...
    def geproblem_laplacian(self, k=4, normalisation='multiplicative', eigens=None, mi=None, nudge=0.5):
        ...
        matrix1 = matrix1 + eye * nudge
        matrix2 = matrix2 + eye * nudge
    ...
    def geproblem_laplacian_reg(self, k=4, normalisation='multiplicative', tau_p=0.1, tau_n=0.1, eigens=None, mi=None):
        """Clusters the graph by solving a laplacian-based generalised eigenvalue problem.
        ...
        Other parameters:
            eigens (int): The number of eigenvectors to use for embedding. For SSBM, k-1 is sufficient when an eigenvalue gap exists, but with high noise, performance is improved by using more.
            mi (int): The maximum number of iterations for which to run eigenvalue solvers.
            solver (string): The eigenvalue solver to use. Currently doesn't do anything because lobpcg is the only one that works.
        ...
        def mv1(v):
            return matrix1.dot(v) + tau_p * v.sum()

        def mv2(v):
            return matrix2.dot(v) + tau_n * v.sum()

        matrix1_o = ss.linalg.LinearOperator(matrix1.shape, matvec=mv1)
        matrix2_o = ss.linalg.LinearOperator(matrix2.shape, matvec=mv2)

        v0 = np.random.normal(0, 1, (self.p.shape[0], eigens))
        (w, v) = ss.linalg.lobpcg(matrix1_o, v0, B=matrix2_o, maxiter=mi, largest=False)
    ...
    def bethe_hessian(self, k, mi=1000, r=None, justpos=True):
...
if __name__ == "__main__":
    ...
    # Ac, truth = SBAM(n, k, p, eta)
    print('Network constructed')
    m = Cluster(Ac)

    eye = ss.eye(m.size, format='csc')
    mat1 = m.p
    mat2 = m.n + eye

    # print(m.findk2())
    #
    # bhpreds = m2.bethe_hessian(k)
    # rscore = sm.adjusted_rand_score(truth, bhpreds)
    # oscore = objscore(bhpreds, k, mat1, mat2)
    # print('BH score is ', rscore)
    # print('     -> BH objective score is ', oscore)
    #
    # # preds = m.normwaggle(k, preds, signed=True)
    # # rscore = sm.adjusted_rand_score(truth, preds)
    # # objscore = score(preds, m.p, m.n, k, obj='bnc')
    # # print('BH score is ', rscore)
    # # print('     -> sym_sep objective score is ', objscore)
    #
    #
    pcapreds = m.spectral_cluster_laplacian(k, normalisation='sym')
    rscore = sm.adjusted_rand_score(truth, pcapreds)
    oscore = objscore(pcapreds, k, mat1, mat2)
    print('PCA score is ', rscore)
    print('     -> PCA objective score is ', oscore)


    pcapreds = m.spectral_cluster_adjacency_reg(k, normalisation='sym')
    rscore = sm.adjusted_rand_score(truth, pcapreds)
    oscore = objscore(pcapreds, k, mat1, mat2)
    print('PCA score is ', rscore)
    print('     -> PCA objective score is ', oscore)
    ...
    print('     -> PCA objective score is ', oscore)
    #
    # pcapreds2 = m2.spectral_cluster_adjacency(k, normalisation='sym')
    # rscore = sm.adjusted_rand_score(truth, pcapreds2)
    # oscore = objscore(pcapreds2, k, mat1, mat2)
    # print('PCA_sym score is ', rscore)
    # print('     -> PCA_sym objective score is ', oscore)
    #
    # pcapreds3 = m.spectral_cluster_adjacency(k, normalisation='sym_sep')
    # rscore = sm.adjusted_rand_score(truth, pcapreds3)
    # oscore = objscore(pcapreds3, k, mat1, mat2)
    # print('PCA_sym_sep score is ', rscore)
    # print('     -> PCA_sym_sep objective score is ', oscore)
    #
    #
    # lappreds = m2.spectral_cluster_laplacian(k, normalisation='sym')
    # rscore = sm.adjusted_rand_score(truth, lappreds)
    # oscore = objscore(lappreds, k, mat1, mat2)
    # print('LAP_sym score is ', rscore)
    # print('     -> LAP_sym objective score is ', oscore)
    #
    # gepreds = m.geproblem_laplacian(k,normalisation='multiplicative')
    # rscore = sm.adjusted_rand_score(truth, gepreds)
    # oscore = objscore(gepreds, k, mat1, mat2)
    # print('GE score is ', rscore)
    # print('     -> GE objective score is ', oscore)
    ...
    gepreds = m.waggle(k, pcapreds,m.normA)
    rscore = sm.adjusted_rand_score(truth, gepreds)
    oscore = objscore(gepreds, k, mat1, mat2)
    print('GE score is ', rscore)
    print('     -> GE objective score is ', oscore)
